...
def logistic_dim_discriminants(X, labels):
    ...
    for morph_dim in morph_dims:
        ...
        endpoints_data = np.concatenate(
            [X[motif_map.get_group(dim).index, :] for dim in morph_dim]
        )
        endpoints_label = np.concatenate(
            [
                np.ones_like(motif_map.get_group(dim).index) * (dim == morph_dim[1])
...
def logistic_dim_reduction(X, labels):
    ...
    proj_matrix = (
        np.array(
            [
                dim_discriminants[dim] / np.linalg.norm(dim_discriminants[dim])
                for dim in dim_discriminants
            ]
        )
        .squeeze()
        .T
...
def generate_neurometric_null_block(
    block_path, num_shuffles, psychometric_params, n_jobs=morphs.parallel.N_JOBS
):
    """
    ...
    block_samples_df = hold_one_out_neurometric_fit_dist_all_subj(
        X_red,
        labels,
        psychometric_params,
        shuffle_count=num_shuffles,
        parallel=True,
        n_jobs=n_jobs,
    )
...
def load_neurometric_null_block(
    block_path, num_shuffles, psychometric_params, n_jobs=morphs.parallel.N_JOBS
):
    """Load pickle containing df of neural rep of a given block path hold_one_out_neurometric_fit_dist_all_subj"""
    nshuffle_dir = morphs.paths.num_shuffle_dir(num_shuffles)
    pkl_path = morphs.paths.num_shuffle_dir(num_shuffles) / (
        morphs.data.parse.blockpath_name(block_path) + ".pkl"
    )
    if not pkl_path.exists():
        generate_neurometric_null_block(
            block_path, num_shuffles, psychometric_params, n_jobs=n_jobs
        )
...
@click.command()
@click.option(
    "--num_shuffles", default=8, help="number of times to shuffle null distribution"
)
@click.option(
