...
def calc_waveforms(block_path):
    ...
    waveforms, cluster_map = ephys.clust.compute_cluster_waveforms_fast(
        block_path, spikes
    )
...
def generate_waveforms(parallel=False, n_jobs=morphs.parallel.N_JOBS):
    if parallel and n_jobs > 1:
        waveforms = Parallel(n_jobs=n_jobs)(
            delayed(calc_waveforms)(block_path) for block_path in morphs.paths.blocks()
        )
    else:
