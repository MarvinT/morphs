...
def reduce_behave_data(df):
    ...
    df = df[
        ["type_", "class_", "response", "correct", "rt", "reward", "stim_id", "subj"]
    ]
...
def gen_behavior_df():
    ...
    behav_data = loading.load_data_pandas(
        morphs.subj.BEHAVE_SUBJS, morphs.paths.behave_data_folder()
    )
    behavior_df = pd.concat(
        [
            morphs.data.parse.behav_data_stim_id(behav_data[subj], subj)
            for subj in morphs.subj.BEHAVE_SUBJS
        ]
    )
...
@morphs.utils.load._load(
    morphs.paths.BEHAVE_PKL,
    gen_behavior_df,
    download_func=morphs.utils.load._download(
        morphs.paths.BEHAVE_PKL, "1wIOg1y0JpyeVgyDFrgOtzYv5XSymA-GX"
    ),
)
