...
def stim_id(df, stim_id='stim_id', end='end', morph_dim='morph_dim',
    ...
    df[end] = df[stim_id].isin(list(b'abcdefghi'))
...
def separate_endpoints(stim_id_series):
    stim_ids = stim_id_series.str.replace(b'[a-i]001', '')
    for motif in b'abcdefgh':
        stim_ids = stim_ids.str.replace(b'[a-i]%s128' % (motif), motif)
    end = stim_ids.isin(list(b'abcdefghi'))
    return stim_ids, end
