...
def morph_viz(spikes, tau=.01, stim_length=.4,
...
def morph_xcor_viz(spikes, tau=.01, stim_length=.4,
                   n_dim=50, title='',
                   row_order='abcdef', col_order='cdefgh'):
    g = sns.FacetGrid(spikes, col='greater_dim', row='lesser_dim',
                      row_order=row_order, col_order=col_order)
    g.map_dataframe(_morph_xcor_viz, tau=tau, stim_length=stim_length,
                    n_dim=n_dim)
    g.set_titles('{row_name}     to     {col_name}')
...
def _morph_xcor_viz(tau=.01, stim_length=.4, n_dim=50, **kwargs):
    t = np.linspace(0, stim_length, n_dim)
    data = kwargs.pop("data")
    ...
    for i, (morph_pos, morph_pos_group) in enumerate(data.groupby('morph_pos')):
        trial_groups = morph_pos_group.groupby(['recording', 'stim_presentation'])
        grid[i, :] = trial_groups['stim_aligned_time'].apply(
            lambda x: morphs.spikes.filtered_response(x.values, tau=tau)(t)).mean()
    ...
    interpolated_grid = sp.interpolate.griddata((x, y), z, (grid_x, grid_y), method='nearest')
    ax = plt.gca()
