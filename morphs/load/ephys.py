...
def calculate_ephys_data(
    block_path, good_clusters=None, collapse_endpoints=False, shuffle_endpoints=False
):
    """
    ...
    for rec, rec_group in stims.groupby("recording"):
        try:
            ...
            if (rec_group["stim_duration"] > 0.41).any():
    ...
    if shuffle_endpoints:
        end_stims = stims[(stims["morph_pos"] == 1) | (stims["morph_pos"] == 128)]
        for morph_pos, morph_pos_group in end_stims.groupby("morph_pos"):
            if morph_pos == 1:
                end_stims.loc[morph_pos_group.index, "end_stim"] = morph_pos_group[
                    "morph_dim"
                ].str[0]
            elif morph_pos == 128:
                end_stims.loc[morph_pos_group.index, "end_stim"] = morph_pos_group[
        ...
        for end_stim, end_stim_group in end_stims.groupby("end_stim"):
            stims.loc[end_stim_group.index, "stim_id"] = end_stim_group[
                "stim_id"
            ].values[np.random.permutation(len(end_stim_group))]

    rigid_pandas.count_events(stims, index="stim_id")

    spikes = spikes.join(
        rigid_pandas.align_events(
            spikes,
            stims,
            columns2copy=[
                "stim_id",
    ...
    spikes["stim_aligned_time"] = (
        spikes["time_samples"].values.astype("int") - spikes["stim_start"].values
    )
