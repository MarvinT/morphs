...
def filtered_response(spk_times, tau=0.01):
    """
    ...
    norm_factor = tau * np.sqrt(2.0 * np.pi)
    return (
        lambda t: np.sum(
            np.exp(-(spk_times - t.reshape((1, -1))) ** 2 / (2 * tau * tau)), 0
        )
        / norm_factor
    )


def create_neural_rep(spikes, stim_length=0.4, max_id_len=None, t=None, num_samples=50):
    """
    ...
    for stim_id, stim_group in spikes.groupby("stim_id"):
        for (rec, stim_pres), trial_group in stim_group.groupby(
            ["recording", "stim_presentation"]
        ):
            ...
            for i, (cluster, cluster_group) in enumerate(cluster_groups):
                X[
                    idx,
                    clust_map[cluster]
                    * num_samples : (clust_map[cluster] + 1)
                    * num_samples,
                ] = temp.values[i]
            labels[idx] = stim_id
            idx += 1
